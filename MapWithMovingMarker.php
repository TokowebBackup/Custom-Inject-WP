...
<!-- Hidden input to store selected dealership title -->
<input type="hidden" id="selected_dealership" name="selected_dealership" />

<script src="https://unpkg.com/leaflet/dist/leaflet.js"></script>
<script src="https://unpkg.com/leaflet-plugins/layer/Marker.SlideTo.js"></script>

<script>
    const carIcons = {}; // To store car model icons

    async function loadCarModelImages() {
        try {
            const response = await fetch('https://aionindonesia.com/wp-json/custom/v1/car-model');
            if (!response.ok) {
            ...
            if (Array.isArray(data)) {
                data.forEach(dealership => {
                    if (dealership.title && dealership.lat && dealership.lng && dealership.address) {
                        ...
                        card.dataset.title = dealership.title; // Store the title in data attribute
                        card.innerHTML = `
                            <div>${dealership.title}</div> <!-- Display only the title -->
                        `;
    ...
    document.getElementById('locations').addEventListener('click', function(e) {
        if (e.target && e.target.closest('.location-card')) {
            ...
            // Update hidden input with selected dealership title only
            const dealershipTitle = selectedCard.dataset.title;
            document.getElementById('selected_dealership').value = dealershipTitle;
        }
    });
    ...
    document.getElementById('car_model').addEventListener('change', function(e) {
        ...
        const iconUrl = carIcons[selectedCarModel]?.url || 'https://aionindonesia.com/wp-content/uploads/2024/08/deaker.svg';
